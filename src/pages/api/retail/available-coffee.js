...
export default async function handler(req, res) {
  ...
  const prisma = new PrismaClient();
      ...
      console.log('🚀 [available-coffee] API called - VERSION 0.1.8 with FINAL MEDIUM BAGS FIX - ALL REFERENCES REMOVED');
  ...
  try {
    ...
    try {
      ...
      shopInventoryCoffee.forEach(item => {
        console.log(`  - ${item.coffee.name}: Small=${item.smallBags || 0}, Small Espresso=${item.smallBagsEspresso || 0}, Small Filter=${item.smallBagsFilter || 0}, Large=${item.largeBags || 0}`);
      });
      ...
      shopInventoryCoffee.forEach(item => {
        console.log(`Adding shop inventory coffee: ${item.coffee.name} - Small: ${item.smallBags || 0}, Small Espresso: ${item.smallBagsEspresso || 0}, Small Filter: ${item.smallBagsFilter || 0}, Large: ${item.largeBags || 0}`);
