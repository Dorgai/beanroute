...
// import { useAuth } from '../context/AuthContext'; // DISABLED FOR MINIMAL TESTING
import {
  Box,
  Button,
  Chip,
...
function OrderDialog({ open, onClose, coffeeItems, selectedShop, haircutPercentage }) {
  ...
  }, [open]); // Only depend on 'open', not 'coffeeItems' to prevent clearing user input

  // Initialize order items when coffeeItems changes, but only if dialog is open and orderItems is empty
  useEffect(() => {
    if (open && Array.isArray(coffeeItems) && coffeeItems.length > 0) {
      // Only initialize if orderItems is empty or doesn't have all coffee items
      setOrderItems(prev => {
        const needsInitialization = Object.keys(prev).length === 0 || 
          !coffeeItems.every(coffee => prev[coffee.id]);
        
        if (needsInitialization) {
          const initialItems = {};
          coffeeItems.forEach(coffee => {
            if (coffee && coffee.id) {
              // Preserve existing values if they exist, otherwise initialize with empty strings
              initialItems[coffee.id] = prev[coffee.id] || { 
                smallBagsEspresso: '', 
                smallBagsFilter: '', 
                largeBags: '' 
              };
          ...
          return { ...prev, ...initialItems };
        }
        
        return prev; // No changes needed
      });
    }
  }, [coffeeItems, open]); // Removed orderItems from dependencies to prevent infinite loop

  // Fetch pending orders data when dialog opens - DISABLED FOR DEBUGGING
  useEffect(() => {
    console.log('[OrdersPage] fetchPendingOrdersData DISABLED for debugging');
  ...
  const handleQuantityChange = (coffeeId, field, value) => {
    ...
    // CRITICAL: Never modify user input - preserve exactly what they typed
    // The user should be able to type anything, including partial numbers, spaces, etc.
    // Only convert to string to ensure consistency, but don't process the content
    const userValue = value === null || value === undefined ? '' : String(value);
    
    // Update the state with the exact user input - NO PROCESSING
    setOrderItems(prev => ({
      ...prev,
      [coffeeId]: {
        ...prev[coffeeId],
    ...
    // Validation happens separately and does NOT modify the input
    // We only validate for display purposes (error messages, etc.)
    // but the actual input value remains exactly what the user typed
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Filter out items with no quantity - mobile-friendly parsing
      const items = Object.entries(orderItems)
        .filter(([coffeeId, item]) => {
          ...
          // Mobile-friendly number parsing with better error handling
          const espressoValue = parseInt(String(item.smallBagsEspresso || '').replace(/[^0-9]/g, '')) || 0;
          const filterValue = parseInt(String(item.smallBagsFilter || '').replace(/[^0-9]/g, '')) || 0;
          const largeBagsValue = parseInt(String(item.largeBags || '').replace(/[^0-9]/g, '')) || 0;
          
          return (espressoValue > 0 || filterValue > 0 || largeBagsValue > 0);
        })
        .map(([coffeeId, item]) => ({
          coffeeId,
          smallBagsEspresso: parseInt(String(item.smallBagsEspresso || '').replace(/[^0-9]/g, '')) || 0,
          smallBagsFilter: parseInt(String(item.smallBagsFilter || '').replace(/[^0-9]/g, '')) || 0,
          largeBags: parseInt(String(item.largeBags || '').replace(/[^0-9]/g, '')) || 0
        }));
      ...
      // Create the order with mobile-optimized timeout and error handling
      ...
      if (typeof onClose === 'function') {
        ...
        // Set a safety timeout to reset loading state if dialog doesn't close
        setTimeout(() => {
          setLoading(false);
        ...
        setLoading(false); // Only reset loading if onClose fails
      }
    } catch (error) {
...
function StatusUpdateDialog({ open, onClose, order, refreshData }) {
  // const { session } = useSession(); // DISABLED - using AuthContext instead
  const user = { role: 'ADMIN' }; // Mock user for debugging
...
export default function RetailOrders() {
  // const { user, loading: authLoading } = useAuth(); // DISABLED FOR MINIMAL TESTING
  const user = { role: 'ADMIN' }; // Mock user for testing
  const authLoading = false; // Mock loading state
  ...
  const [showMinInventoryAlert, setShowMinInventoryAlert] = useState(false);
  const [showChangedOrdersAlert, setShowChangedOrdersAlert] = useState(false);
  const [hasShownInventoryAlert, setHasShownInventoryAlert] = useState(false);
  const [hasShownChangedOrdersAlert, setHasShownChangedOrdersAlert] = useState(false);
  ...
  // Handle authentication redirect - only redirect once
  useEffect(() => {
    console.log('[OrdersPage] Auth redirect DISABLED for debugging');
    // TEMPORARILY DISABLED TO DEBUG INFINITE REFRESH
    // if (!authLoading && !user && !isRedirecting) {
    //   console.log('[OrdersPage] No user found, redirecting to login');
    //   setIsRedirecting(true);
    //   router.push('/login');
    // }
  }, []); // Removed dependencies since redirect is disabled

  // Show loading state while authentication is being checked
  if (authLoading) {
    console.log('[OrdersPage] Auth loading, showing spinner');
    return (
  ...
  // Show loading state while redirecting to prevent infinite loop
  if (!user || isRedirecting) {
    console.log('[OrdersPage] No user or redirecting, showing loading');
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500"></div>
      </div>
  ...
  // Show minimum inventory alert when shop changes, then auto-hide after 5 seconds
  useEffect(() => {
    if (selectedShopDetails && (canUpdateInventory || isAdmin || isOwner) && !hasShownInventoryAlert) {
      setShowMinInventoryAlert(true);
      setHasShownInventoryAlert(true);
      
      // Auto-hide after 5 seconds
      const timer = setTimeout(() => {
        setShowMinInventoryAlert(false);
      }, 5000);
      
      return () => clearTimeout(timer);
    }
  }, [selectedShopDetails, canUpdateInventory, isAdmin, isOwner, hasShownInventoryAlert]);

  // Show alert when there are new messages, then auto-hide after 5 seconds
  useEffect(() => {
    if (alertMessage && !hasShownInventoryAlert) {
      setShowMinInventoryAlert(true);
      setHasShownInventoryAlert(true);
  ...
  }, [alertMessage, hasShownInventoryAlert]);

  // Show changed orders alert when there are recent changes, then auto-hide after 5 seconds
  useEffect(() => {
    if (recentlyChangedOrders.length > 0 && !hasShownChangedOrdersAlert) {
      setShowChangedOrdersAlert(true);
      setHasShownChangedOrdersAlert(true);
      
      // Auto-hide after 5 seconds
      const timer = setTimeout(() => {
        setShowChangedOrdersAlert(false);
      }, 5000);
      
      return () => clearTimeout(timer);
    }
  }, [recentlyChangedOrders, hasShownChangedOrdersAlert]);
  ...
  // Fetch haircut percentage - DISABLED FOR DEBUGGING
  const fetchHaircutPercentage = async () => {
    console.log('[OrdersPage] fetchHaircutPercentage DISABLED for debugging');
    return; // DISABLED FOR DEBUGGING
    /*
    try {
  ...
  useEffect(() => {
    ...
    fetchShops();
  }, []); // Removed selectedShop dependency
  ...
  useEffect(() => {
    ...
    fetchShopDetails();
  }, [selectedShop]); // Restored selectedShop dependency
  ...
  // Process orders to check for pending orders and recent changes - DISABLED FOR DEBUGGING
  useEffect(() => {
    console.log('[OrdersPage] processOrders DISABLED for debugging');
    return; // DISABLED FOR DEBUGGING
    /*
    ...
    */
  }, []); // Removed orders dependency

  // Fetch inventory history when shop changes - DISABLED FOR DEBUGGING
  useEffect(() => {
    console.log('[OrdersPage] fetchInventoryHistory DISABLED for debugging');
    return; // DISABLED FOR DEBUGGING
    /*
    if (!selectedShop) return;
  ...
  // Fetch recent alert logs when shop changes - DISABLED FOR DEBUGGING
  useEffect(() => {
    console.log('[OrdersPage] fetchRecentAlertLogs DISABLED for debugging');
    return; // DISABLED FOR DEBUGGING
    /*
  ...
  }, [tabIndex]); // Restored tabIndex dependency
  ...
  // Update global count variable whenever availableCoffee changes - DISABLED FOR DEBUGGING
  useEffect(() => {
    console.log('[OrdersPage] updateGlobalCount DISABLED for debugging');
    return; // DISABLED FOR DEBUGGING
    /*
    ...
    */
  }, []); // Removed availableCoffee dependency

  // if (!session) {
  //   return null;
  // }
  // DISABLED - using AuthContext instead
    ...
    <>
      ...
      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        ...
        <Paper elevation={0} sx={{ 
          ...
          {shouldShowChangedOrdersAlert && recentlyChangedOrders.length > 0 && showChangedOrdersAlert && (
            <IconlessAlert severity="info" sx={{ mb: 3 }}>
              <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
              ...
              <Box role="tabpanel" hidden={tabIndex !== 0} id="tabpanel-0" aria-labelledby="tab-0" sx={{ py: 2 }}>
                {loading ? (
                  ...
                  <>
                    ...
                    {selectedShopDetails && (canUpdateInventory || isAdmin || isOwner) && showMinInventoryAlert && (
                      <IconlessAlert severity="info" sx={{ mb: 2 }}>
                        <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                    ...
                    {(isAdmin || isOwner) && (
                      <Box sx={{ mt: 4 }}>
                        ...
                        {recentAlertLogs.length > 0 ? (
                          <TableContainer component={Paper} variant="outlined" sx={{ mb: 2, backgroundColor: 'transparent' }}>
                            <Table size="small">
                              <TableHead>
                                <TableRow>
